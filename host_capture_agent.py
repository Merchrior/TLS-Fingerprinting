...
class HostCaptureAgent:
    ...
    def _log(self, level: str, message: str) -> None:
        ...
        # ÖNEMLİ:
        # Host agent artık SQLite'a yazmıyor.
        # Böylece host + container aynı DB'ye yazıp çakışmıyor.

    def _write_json_atomic(self, path: Path, data: dict | list) -> None:
    ...
    def _get_config(self) -> dict:
        ...
        ring_duration_raw = self.db.get_config("ring_duration", "30") or "30"
        ring_files_raw = self.db.get_config("ring_files", "10") or "10"

        try:
            ring_duration = int(ring_duration_raw)
        except ValueError:
            ring_duration = 30

        try:
            ring_files = int(ring_files_raw)
        except ValueError:
            ring_files = 10
    ...
    def get_detected_interfaces(self) -> list[dict]:
        ...
        try:
            ...
            self._log("ERROR", f"Failed to list interfaces with TShark: {e}")
            return []
    ...
    def sync_interfaces_to_file(self) -> None:
        ...
        self._log("INFO", f"Detected interfaces written to {self.interfaces_file} ({len(interfaces)} entries)")

    def write_agent_status(self, status: str, message: str, config: dict | None = None) -> None:
        payload = {
            "status": status,
            "message": message,
    ...
    def stop_capture(self) -> None:
        if self.capture_process and self.capture_process.poll() is None:
            self._log("INFO", "Stopping current TShark capture process")
    ...
    def watch_for_config_changes(self) -> None:
        cfg = self._get_config()
        new_signature = self._build_signature(cfg)
        ...
        if new_signature != self.current_signature:
            self._log("INFO", "Detected config change in DB, restarting capture")
        ...
        try:
            while True:
                ...
                if self.capture_process and self.capture_process.poll() is not None:
                    self._log("WARNING", "TShark process exited unexpectedly, retrying start")
                    time.sleep(2)
                    self.start_capture()
