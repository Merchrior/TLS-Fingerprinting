...
class UIDatabaseAdapter:
        ...
        try:
            with psycopg2.connect(self.conn_str) as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    # ÇÖZÜM: Parametre yoksa dümdüz çalıştır, psycopg2 % işaretlerine karışmasın!
                    if params:
    ...
    def get_summary_metrics(self):
        ...
        white = self._fetch("SELECT COUNT(*) as c FROM whitelist")
        
        t_val = total[0]['c'] if total else 0
        k_val = known[0]['c'] if known else 0
        p_val = pcaps[0]['c'] if pcaps else 0
        
        return {
            "total_events": t_val,
            "known_events": k_val,
            "unknown_events": t_val - k_val,
            "active_pcap_jobs": is_active,  # Artık butona basınca Active'e geçecek!
            "processed_pcap_count": p_val,  # Artık gerçek sayıyı gösterecek!
            "candidate_count": cand[0]['c'] if cand else 0, 
            "whitelist_count": white[0]['c'] if white else 0
        }

    def get_recent_events(self, limit=50):
        rows = self._fetch("SELECT * FROM tls_events ORDER BY timestamp DESC LIMIT %s", (limit,))
        safe_rows = []
        for r in rows:
            ...
            safe_rows.append({
                ...
                "sni": r.get('sni', 'Unknown'),
                "ja3_hash": r.get('ja3_hash', '-'),
                "prediction": r.get('prediction', '-'),
                "confidence": float(conf) if conf is not None else 0.0,
                "status": r.get('threat_level', 'Unknown')
            })
        return safe_rows
    ...
    def get_event_trend(self, limit=24):
        query = "SELECT date_trunc('hour', timestamp) as hour_bucket, COUNT(*) as event_count FROM tls_events GROUP BY hour_bucket ORDER BY hour_bucket DESC LIMIT %s"
        return self._fetch(query, (limit,))

    def get_port_distribution(self, limit=10):
        return self._fetch("SELECT dst_port, COUNT(*) as hit_count FROM tls_events WHERE dst_port IS NOT NULL GROUP BY dst_port ORDER BY hit_count DESC LIMIT %s", (limit,))
    ...
    def get_last_processed_pcap(self):
        # 🚀 ÇÖZÜM 3: Tamamen esnek Regex arama (Hata payı 0)
        res = self._fetch("SELECT timestamp, message FROM system_logs WHERE message ILIKE %s ORDER BY timestamp DESC LIMIT 1", ('%Processed PCAP%',))
        if res:
            msg = res[0]['message']
            try:
                import re
                match = re.search(r'Processed PCAP:\s*([^\s\(]+)', msg, re.IGNORECASE)
                filename = match.group(1) if match else "Unknown"
            except:
                filename = "Unknown"
            return {"file_name": filename, "processed_at": res[0]['timestamp']}
        return None

    def get_pcap_files(self, limit=10, status=None):
        limit_val = int(limit) if limit else 10
        res = self._fetch(f"SELECT timestamp, message FROM system_logs WHERE message ILIKE %s ORDER BY timestamp DESC LIMIT {limit_val}", ('%Processed PCAP%',))
        files = []
        import re
        for r in res:
            msg = r['message']
            try:
                file_match = re.search(r'Processed PCAP:\s*([^\s\(]+)', msg, re.IGNORECASE)
                filename = file_match.group(1) if file_match else msg
                rec_match = re.search(r'\((\d+)\s*events\)', msg, re.IGNORECASE)
                records = int(rec_match.group(1)) if rec_match else 0
            except:
                filename, records = msg, 0
    ...
    def set_config(self, key, value):
        ...
        self._execute("INSERT INTO system_config (key, value) VALUES (%s, %s) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;", (key, value))
...
def render_overview(db: UIDatabaseAdapter, table_limit: int) -> None:
    ...
    last_pcap_data = db.get_last_processed_pcap()
    last_pcap      = last_pcap_data["file_name"] if last_pcap_data else "No processed file yet"
