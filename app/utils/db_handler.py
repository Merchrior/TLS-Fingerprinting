...
class DatabaseManager:
    ...
    Amaç:
    - TLS event kayıtlarını tutmak
    - Bilinen JA3 whitelist kaydını tutmak
    - Unknown / candidate tahminlerini tutmak
    - PCAP işleme durumlarını tutmak
    - Uygulama loglarını tutmak
    - Uygulama ayarlarını tutmak
    - Dashboard için özet veri sağlamak
    """
    ...
    def _initialize_database(self) -> None:
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_tls_events_timestamp
                ON tls_events(timestamp DESC)
            """)

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_app_commands_status
                ON app_commands(status)
            """)

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_app_commands_created_at
                ON app_commands(created_at DESC)
            """)

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_tls_events_ja3_hash
                ON tls_events(ja3_hash)
            """)

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_whitelist_ja3_hash
                ON whitelist(ja3_hash)
            """)

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_candidates_ja3_hash
                ON candidates(ja3_hash)
            """)

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_pcap_files_status
                ON pcap_files(status)
            """)

            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_pcap_files_first_seen
                ON pcap_files(first_seen DESC)
            """)
    ...
    def enqueue_command(self, command_name: str, payload: Optional[str] = None) -> int:
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO app_commands (command_name, payload, status)
                VALUES (?, ?, 'pending')
    ...
    def get_candidates(self, limit: int = 50) -> List[Dict[str, Any]]:
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                SELECT
                    id, ja3_hash, ja3_string, predicted_app,
    ...
    def promote_candidate_to_whitelist(
        ...
        with self._get_connection() as conn:
            ...
            """, (
                candidate["ja3_hash"],
                candidate["ja3_string"],
                final_app_name,
                "known",
                candidate["confidence"],
                source,
                "Promoted from candidates table"
            ))

            cursor.execute("""
                UPDATE candidates
                SET promoted = 1, last_seen = CURRENT_TIMESTAMP
                WHERE ja3_hash = ?
            """, (ja3_hash,))
    ...
    def get_top_predictions(self, limit: int = 10) -> List[Dict[str, Any]]:
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                SELECT
                    COALESCE(prediction, 'Unknown') AS prediction,
                    COUNT(*) AS hit_count,
                    MAX(timestamp) AS last_seen
                FROM tls_events
    ...
    def seed_sample_whitelist(self) -> None:
        sample_entries = [
            {
                "ja3_hash": "example_chrome_hash",
                "ja3_string": "771,4865-4866-4867,0-11-10-35-16,29-23-24,0",
                "app_name": "Chrome-like",
                "category": "browser",
                "confidence": 99.0,
                "source": "demo-seed",
                "notes": "Sample whitelist entry"
            },
            {
                "ja3_hash": "example_firefox_hash",
                "ja3_string": "771,4865-4867,0-10-11-13-16,29-23-24,0",
                "app_name": "Firefox-like",
                "category": "browser",
                "confidence": 99.0,
                "source": "demo-seed",
                "notes": "Sample whitelist entry"
